...
public class GUIQuanLyDichVu extends JPanel implements ActionListener, MouseListener {
	...
	public GUIQuanLyDichVu(Frame parent) {
		...
		txtMa.setPreferredSize(new Dimension(0, 30));
		/// chu y
		...
		cmbTheLoai.setPreferredSize(new Dimension(60, 30));
...
//			child.setVisible(false);
	...
	private void actionXoa() {
		...
		DichVu dv;
		String ma = table.getValueAt(row, 0).toString();
		if (JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn xóa dịch vụ này?", "Cảnh Báo",
	...
	private void actionLuu() {
		...
		if (chucNangHienTai.equals("them")) {
			String type = "Insert";
			if (checkData("", ten, theLoai, donVi, donGia, soLuong)) {
				if (qldv.curdDichVu(ma, ten, donVi, theLoai, Integer.parseInt(soLuong), Integer.parseInt(donGia),
						type)) {
					// Object[] rowdata = {ma, ten,theLoai,donGia,soLuong,donVi,ghiChu};
					// tableModel.addRow(rowdata);
					updateTableData();
	...
	private void updateTableData1(ArrayList<DichVu> list) {
		...
		DecimalFormat df = new DecimalFormat();
		for (DichVu dv : list) {
			String[] rowData = { dv.getMaDV() + "", dv.getTenDV(), dv.getDonVi(), dv.getLoai(), dv.getSoLuongCo() + "",
					df.format(dv.getGiaDV()) };
			tableModel.addRow(rowData);
		}
		table.setModel(tableModel);
	}
