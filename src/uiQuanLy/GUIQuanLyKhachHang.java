...
public class GUIQuanLyKhachHang extends JPanel implements ActionListener, MouseListener {
	...
	public GUIQuanLyKhachHang(Frame parent) {
		...
		txtMa.setPreferredSize(new Dimension(80, 30));
		txtMa.setMaximumSize(new Dimension(80, 30));
		...
		txtSDT.setPreferredSize(new Dimension(80, 30));
		...
		b1.add(Box.createHorizontalStrut(5));
		b1.add(btnThoat = new JButton("Thoát", new ImageIcon(".\\image\\btn-exit.png")));
		b1.add(Box.createHorizontalStrut(5));
		...
		bc.add(b1 = Box.createHorizontalBox());
		b1.add(Box.createHorizontalStrut(50));
		b1.add(btnBack = new JButton("Lùi Về Trang Trước", new ImageIcon(".\\image\\btn-back.png")));
...
//		 txtDonGia.setText("");
	...
	private void actionXoa() {
		...
		if (JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn xóa Khách Hàng này?", "Cảnh Báo",
				JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
			qlkh.curdKhachHang(ma, "", "", "1999-08-28", 0, "", "Delete");
...
//			txtDonGia.requestFocus();
	...
	private void actionLuu() {
		...
		try {
			...
			if (chucNangHienTai.equals("them")) {
				String type = "Insert";
				// String ma, String ten,String theLoai,String donVi,String donGia,String
				// soLuong
	...
	// public ArrayList<KhachHang> timKiem(String maKH, String hoTen, String CMND,
	...
	public void actionTim(String maKH, String hoTen, String CMND, String ngaySinh, String gioiTinh, String sDT) {
		try {
			...
			// String maDV, String tenDV, String donVi, String loai, String soLuongCo,
			// String giaDV
			ArrayList<KhachHang> list = qlkh.timKiem(maKH, hoTen, CMND, ngaySinh, gioiTinh, sDT);
			if (list.size() == 0) {
				JOptionPane.showMessageDialog(this, "Không Tìm Thấy");
			} else
				updateTableData1(list);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void updateTableData1(ArrayList<KhachHang> list) {
		...
		for (KhachHang kh : list) {
			...
			if (kh.getGioiTinh() == 1) {
				...
				gioiTinh = "Nam";
			String[] rowData = { kh.getMa() + "", kh.getHoTen(), kh.getcMND(), kh.getsDT(), gioiTinh,
	...
	private void updateTableData() {
		...
		for (KhachHang dv : list) {
			...
			if (dv.getGioiTinh() == 1) {
				...
				gioiTinh = "Nam";
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			String[] rowData = { dv.getMa() + "", dv.getHoTen(), dv.getcMND(), dv.getsDT(), gioiTinh,
					sdf.format(dv.getNgaySinh()) };
			tableModel.addRow(rowData);
		}
		table.setModel(tableModel);
	}
