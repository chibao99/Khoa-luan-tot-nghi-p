	...
	private void them2() {
		try {
			...
			if(!Pattern.matches("\\d{9}", txtCmnd.getText().trim())) {
				JOptionPane.showMessageDialog(null, "CMND phải 9 số",
						"Thông báo", 0, new ImageIcon(".\\image\\error.png"));
				throw new Exception();
			}else if(!Pattern.matches("\\d{10}", txtSoDienThoai.getText().trim())) {
				JOptionPane.showMessageDialog(null, "Số Điẹn thoại phải 10 chữ số",
						"Thông báo", 0, new ImageIcon(".\\image\\error.png"));
				throw new Exception();
			}else if(Pattern.matches("[0-9]+", txtTenNhanVien.getText().trim())) {
				JOptionPane.showMessageDialog(null, "Tên nhân viên không chứa số",
						"Thông báo", 0, new ImageIcon(".\\image\\error.png"));
				throw new Exception();
			}
